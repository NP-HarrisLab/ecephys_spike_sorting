...
def EphysParams(metaFullPath):
    ...
    uVPerBit = Chan0_uVPerBit(meta, metaFullPath, probe_type)
...
#
def Chan0_uVPerBit(meta, metaFullPath, probe_type):
...
def CreateNITimeEvents(catGT_run_name, gate_string, catGT_dest):
    ...
    fyi_all_path = Path(
        os.path.join(
            catGT_dest,
            output_folder,
            catGT_run_name + "_g" + gate_string + "_all_fyi.txt",
        )
    )
    fyi_single_path = Path(
        os.path.join(
            catGT_dest, output_folder, catGT_run_name + "_g" + gate_string + "_fyi.txt"
        )
    )
    fyi_exists = False
