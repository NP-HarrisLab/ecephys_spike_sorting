...
def call_TPrime(args):
    ...
    if all_fyi_exists or fyi_exists:
        ...
        if toStream_js == 2:
            ...
            for ks_outdir in ks_outdir_iter:
                ...
                st_file = os.path.join(
                    run_directory, prb_dir, ks_outdir, "spike_times.npy"
                )
                ...
                shutil.copyfile(
                    toStream_events_sec,
                    os.path.join(
                        run_directory, prb_dir, ks_outdir, "spike_times_sec_adj.npy"
                    ),
                )
        ...
        print(
            "You can generate a new fyi file by running an extract only pass on the CatGT output."
        )
        print(
            "Typical extract only command line also see CatGT Readme under: -t=cat defer extraction to a later pass "
        )
        print(
            "-dir=<catGT_dest> -run=catgt_<run_name> -g=0 -t=cat -ap -ni -prb=0 -prb_fld -no_tshift ^ "
        )
    ...
    if sys.platform.startswith("win"):
        exe_path = os.path.join(
            args["tPrime_helper_params"]["tPrime_path"], "runit.bat"
        )
    ...
    tcmd = (
        exe_path + " -syncperiod=" + repr(sync_period) + " -tostream=" + toStream_path
    )
...
def create_PSTH_events(all_list, out_list, prbDir_list, extract_str, sort_name):
    ...
    ex_type, stream_index, prb_index, ex_name_str = catGT_ex_params_from_str(
        extract_str
    )
