...
from ._schemas import KilosortHelperSchema
...
def run_kilosort(args):
    ...
    commit_date, commit_time = get_repo_commit_date_and_hash(
        args["kilosort_helper_params"]["kilosort_repository"]
    )
    ...
    if args["kilosort_helper_params"]["spikeGLX_data"]:
        ...
        metaName, _ = os.path.splitext(args["ephys_params"]["ap_band_file"])
        ...
        connected = MetaToCoords(
            metaFullPath=metaFullPath,
            outType=1,
            badChan=MaskChannels,
        ...
        _, _, _, surface_channel, _ = read_probe_json(
        ...
        top_channel = np.min(
            [
                args["ephys_params"]["num_channels"],
                int(surface_channel)
                + args["kilosort_helper_params"]["surface_channel_buffer"],
            ]
        )

        matlab_file_generator.create_chanmap(
            args["kilosort_helper_params"]["matlab_home_directory"],
            EndChan=top_channel,
    ...
    shutil.copyfile(
        os.path.join(
            args["directories"]["ecephys_directory"],
            "modules",
            "kilosort_helper",
    ...
    if args["kilosort_helper_params"]["kilosort_version"] == 1:
        ...
        matlab_file_generator.create_config(
            args["kilosort_helper_params"]["matlab_home_directory"],
        ...
        matlab_file_generator.create_config2(
            args["kilosort_helper_params"]["matlab_home_directory"],
    ...
    if args["kilosort_helper_params"]["kilosort_version"] == 1:
        ...
        eng.addpath(eng.genpath(NPY_dir))
        eng.addpath(home_dir)
        eng.main_kilosort_multiversion(
            args["kilosort_helper_params"]["kilosort2_params"]["KSver"],
    ...
    if copy_fproc:
        ...
        if args["kilosort_helper_params"]["kilosort2_params"]["KSver"] in [
            "2.5",
            "3.0",
        ]:
...
def get_noise_channels(
    raw_data_file, num_channels, sample_rate, bit_volts, noise_threshold=20
):
...
def fix_phy_params(output_dir, dat_path, dat_name, chan_phy_binary, sample_rate):
    ...
    shutil.copy(
        os.path.join(output_dir, "params.py"), os.path.join(output_dir, "old_params.py")
    )
...
def main():
    ...
    args = KilosortHelperSchema().load({"input": input_json, "output": {}})

    output = run_kilosort(args["input"])
