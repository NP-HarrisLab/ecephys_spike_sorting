...
from ecephys_spike_sorting.common.utils import find_range, printProgressBar, rms
...
def compute_channel_offsets(ap_data, ephys_params, params, xCoord, yCoord):
    ...
    for i in range(numIterations):
        ...
        start_sample = int(
            (params["start_time"] + params["skip_s_per_pass"] * i)
            * ephys_params["sample_rate"]
        )
        end_sample = start_sample + int(
            params["time_interval"] * ephys_params["sample_rate"]
        )
    ...
    output_dict = {
        ...
        "vertical_pos": 20
        * (np.floor(np.arange(0, numChannels) / 2) + 1).astype("int"),
...
def find_surface_channel(lfp_data, ephys_params, params, xCoord, yCoord, shankInd):
    ...
    in_saline_range = np.squeeze(
        (chan_y > saline_range[0]) & (chan_y < saline_range[1])
    )
    saline_chan = np.where(in_saline_range)[0]
    if saline_chan.any() == False:
        print(
            "No channels in saline depth range -- saline median will not be subtracted."
        )
    ...
    for p in range(passes_used):
        ...
        for ch in np.arange(nchannels_used):
            ...
            sample_frequencies, Pxx_den = welch(
                chunk[:, ch], fs=sample_frequency, nfft=nfft
            )
            power[:, ch] = Pxx_den
        ...
        surface_channels = np.where(
            (np.diff(values) < diff_thresh) * (values[:-1] < power_thresh)
        )[0]
    ...
    output_dict = {"surface_y": surface_y, "air_y": air_y}
...
def plot_results(
    ...
    plt.plot(
        [np.amin(sorted_y), np.amax(sorted_y)], [power_thresh, power_thresh], "--k"
    )
